...
// ==============================================================================
const theme = {
  primary: '#0F172A',      // Azul Marinho Escuro (Fundo principal)
  cardBg: '#1E293B',       // Azul Marinho Claro (Cartões)
  gold: '#D4AF37',         // Dourado
  goldLight: '#FDE68A',    
  
  secondary: '#94A3B8',    // Textos secundários
...
// PALETA PARA OS GRÁFICOS (Adaptada para fundo escuro)
const CHART_COLORS = [
  '#D4AF37', '#38BDF8', '#10B981', '#F59E0B', '#A78BFA', '#F472B6', '#818CF8', '#94A3B8'
];
...
export function FinanceDashboard() {
  ...
  const generatePDF = async () => {
     if (user?.plan !== 'PLUS') {
         Alert.alert("Premium 🏆", "Faça o upgrade para gerar relatórios financeiros profissionais com a logo do Kairon.");
  ...
  const renderPeriodSelector = () => (
    <View style={styles.periodContainer}>
      {(['day', 'week', 'month', 'year'] as Period[]).map((p) => {
        ...
        const isLocked = user?.plan !== 'PLUS' && (p === 'year' || p === 'week');
        ...
        return (
          <TouchableOpacity
            ...
            onPress={() => {
                if (isLocked) {
                    Alert.alert("Premium 🏆", "Acesso ao histórico completo disponível no plano PLUS.");
                    navigation.navigate('SubscriptionScreen');
                } else {
                    setPeriod(p);
                }
      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        >
              ...
              <TouchableOpacity style={styles.reportButton} onPress={generatePDF} disabled={generatingPdf} activeOpacity={0.8}>
                {generatingPdf ? (
                    ...
                    <Image 
                        ...
                        style={{ width: 22, height: 22, tintColor: theme.gold }} 
                        resizeMode="contain"
                    />
                )}
              </TouchableOpacity>
          ...
          <View style={styles.mainContent}>
              ...
              {revenueData.values.length > 0 && !revenueData.values.every(v => v === 0) ? (
                <LineChart
                  ...
                  chartConfig={{
                    ...
                    decimalPlaces: 0, 
                    color: (opacity = 1) => `rgba(212, 175, 55, ${opacity})`, // Linha Dourada
            ...
            {/* GRÁFICO 2: DESPESAS (COM CADEADO 🔒) */}
            <View style={styles.chartCard}>
              <View style={styles.chartHeader}>
              ...
              <PremiumGate description="Veja exatamente onde você está gastando e aumente seu lucro.">
